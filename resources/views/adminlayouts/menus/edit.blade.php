...
@section('content')
    <div class="page-wrapper">
        ...
        <div class="container-fluid">
            @include('adminlayouts.partials.content-header', ['name' => 'Menu', 'key' => 'Edit', 'link' => route('menus.index')])

            <form class="form-horizontal r-separator" action="{{route('menus.update', ['id' => $menu->id])}}" method="post">
                @csrf
                    ...
                    <div class="form-group row align-items-center m-b-0">
                        ...
                        <div class="col-9 border-left p-b-10 p-t-10">
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name"
                                   name="name" value="{{$menu->name}}" placeholder="Menu Name">
                            @error('name')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    </div>
                    <div class="form-group row align-items-center m-b-0 @error('name') is-invalid @enderror">
