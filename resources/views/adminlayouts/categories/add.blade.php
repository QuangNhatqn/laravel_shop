...
@section('title')
    <title>Add {{ucwords($type)}} Category</title>
@endsection
...
@section('content')
    <div class="page-wrapper">
        ...
        <div class="container-fluid">
            @include('adminlayouts.partials.content-header', ['name' => ucwords($type) . ' Category', 'key' => 'Add', 'link' => route('categories.index', ['type' => $type])])

            <form class="form-horizontal r-separator" action="{{route('categories.store')}}" method="post">
                @csrf
                <div class="card-body bg-light">
                    <div class="form-group row align-items-center m-b-0">
                        <label for="name" class="col-3 text-right control-label col-form-label">Category</label>
                    ...
                    <div class="form-group row align-items-center m-b-0">
                        ...
                        <div class="col-9 border-left p-b-10 p-t-10">
                            <input type="text" class="form-control @error('slug') is-invalid @enderror"
                                   id="slug" name="slug" placeholder="Example: tin-tuc" value="{{old('slug')}}">
                            @error('slug')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div style="display: none"><input name="type" value="{{$type}}"></div>
